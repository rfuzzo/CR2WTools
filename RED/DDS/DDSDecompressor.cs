...
namespace RED.DDS
{
    internal class DDSDecompressor
    {
        ...
        private static byte[] DecompressDXT2(DDSStruct header, byte[] data, DDSPixelFormat pixelFormat)
        {
            // allocate bitmap
            int width = (int)header.width;
            int height = (int)header.height;
        ...
        private static unsafe byte[] DecompressDXT5(DDSStruct header, byte[] data, DDSPixelFormat pixelFormat)
        {
            // allocate bitmap
            int bpp = (int)(DDSHelper.PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
            int bps = (int)(header.width * bpp * DDSHelper.PixelFormatToBpc(pixelFormat));
        ...
        private static unsafe byte[] DecompressRGB(DDSStruct header, byte[] data, DDSPixelFormat pixelFormat)
        {
            // allocate bitmap
            int bpp = (int)(DDSHelper.PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
            int bps = (int)(header.width * bpp * DDSHelper.PixelFormatToBpc(pixelFormat));
